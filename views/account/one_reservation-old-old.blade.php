...
{{--                                    <li class="ch-breadcrumb__item">{{ $reservation->reference }} (<a href="/vhc/{{ $reservation->health_check->uuid }}">{{ \App\VHC::mapStatusText($reservation->health_check->status) }}</a>)</li>--}}
                                @endif
                                @endif
                                    @if($reservation->type == "hire" && $reservation->payment_status == "unpaid" &&  $reservation->payment_method == 'stripe' && ($reservation->status !== 'cancelled' && $reservation->status !== 'completed' ))
                    ...
                    <div class="ch-container">
                        <div class="ch-row sl:ch-ph--1 ch-mt--4">
                            <ul class="ch-breadcrumb">
                                <li class="ch-breadcrumb__item"><a href="{{ route('account') }}">Account</a></li>
                                <li class="ch-breadcrumb__item"><a href="{{ route('account.reservations') }}">Bookings</a></li>
                                @if($reservation->type == "hire")
                                    <li class="ch-breadcrumb__item ch-overflow--auto">{{ $reservation->reference }} ({{ $pageTagline }})</li>
                                @else

                                    @if($reservation->status == 'cancelled')
                                        <li class="ch-breadcrumb__item">{{ $reservation->reference }} (Cancelled)</li>
                                    @else
                                        ...
                                        @else
                                            <li class="ch-breadcrumb__item">{{ $reservation->reference }} ({{ \App\VHC::mapStatusText($reservation->health_check->status) }})</li>
                                        @endif
                                        @else
                                            <li class="ch-breadcrumb__item">{{ $reservation->reference }}</li>
                                        @endif
                                    @endif
                                @endif
                            </ul>
                        ...
                        @foreach (['danger', 'warning', 'success', 'info'] as $msg)
                            @if(Session::has('alert-' . $msg))
                                <div class="ch-row sl:ch-ph--1 ch-mt--4">
                                    <div class="ch-alert ch-alert--{{ $msg }}">
...
{{--                                        </button>--}}
                            ...
                            @if($reservation->car->image)
                                    <div class="sm:ch-col--6 sm:ch-col--offset-3 md:ch-col--4 md:ch-col--offset-0 ch-mb--3">
                                        <img src="{{ \Storage::disk('public')->url($reservation->car->image) }}" height="200" style="margin-top: -20px;"/>
                                    </div>
                            @endif

                                <div class="ch-text--left sl:ch-text--left ch-pl--2 ch-mb--3">
                                    <h1 class="ch-fs--5 md:ch-fs--5">{{ $reservation->car->make }} {{ $reservation->car->model }} {{ $reservation->car->colour }} ({{ $reservation->car->fuel_type }})</h1>

                                    @if($reservation->health_check && $reservation->health_check->mileage)
                                    <h2 class="ch-fs--3">Mileage: {{ $reservation->health_check->mileage }}</h2>
                                    @endif

                                    <h2 class="ch-fs--3">Colour: {{ $reservation->car->colour }}</h2>
                                    <h2 class="ch-fs--3">First Registered: {{ $reservation->car->registered_date->format('d/m/Y') }}</h2>
                                    <h2 class="ch-fs--3">MOT Due: {{ $reservation->car->mot_due_date->format('d/m/Y') }}</h2>
                                        ...
                                        <tr class="cart_item">
                                            <td class="product-name">
                                                ...
                                                <dl class="variation">
                                                    ...
                                                    <dd class="variation-PickUpDropOffLocation ch-text--left ch-ml--0"><p>
                                                        {{ $reservation->car->location->name }} ({{ $reservation->car->location->address }})</p></dd>
                                                    ...
                                                    <dd class="variation-PickupDateampTime ch-text--left ch-ml--0"><p>
                                                        {{ \Carbon\Carbon::parse($reservation->pickup_date)->format('d M Y (h A)') }}</p>
                                                    </dd>
...
{{--                                                        </dd>--}}
                        ...
                        <div class="ch-row ch-mt--5">
                            ...
                            @else
                                ...
                                <div class="ch-col--12">
                                    <div id="map">
                                        <div class="map-marker" data-title="{{ $reservation->location->name }}" data-latlng="{{ $reservation->location->lat }},{{ $reservation->location->lng }}"></div>
                                    </div>
                                </div>
                            @endif
                        </div>
        ...
        jQuery(document).ready(function(){
            jQuery("#map").simplegmaps({
                ...
                MapOptions: {
                    ...
                    @else
                    center: '{{ $reservation->location->lat }},{{ $reservation->location->lng }}',
                    @endif
                    scrollwheel: false,
            ...
            var marker = new google.maps.Marker({position: '{{ $reservation->car->location->lat }},{{ $reservation->car->location->lng }}', map: map});
                    @else
            var marker = new google.maps.Marker({position: '{{ $reservation->location->lat }},{{ $reservation->location->lng }}', map: map});
            @endif
