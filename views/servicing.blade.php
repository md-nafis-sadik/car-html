...
@section('main')
    @component('components.hero-section', ['name' => 'Manufacturer Servicing', 'image' => 'https://autofusioncars.co.uk/images/manufacturer-heros/honda.jpeg'])
        @slot('caption')
            At Autofusion, you never need to worry about whether <br class="desktop"> our team of mechanics will know how to take care of your vehicle.
        @endslot
    ...
    <section class="dark-container !bg-[#F3F4F6] mt-[76px] mb-9">
        ...
        <p class="max-w-[903px] text-dark text-opacity-60 text-base sm:text-[23px] leading-7 font-medium text-center mx-auto mt-[30px] mb-[110px]"
           data-aos="fade-up">
            Our experts are on hand to ensure that the servicing of your car goes by the book as laid out by the manufacturer.
        </p>
            ...
            @component('components.section-summary')
                @slot('caption')
                    <span class="text-dark">
                        They will only use parts that are genuine, and they will go by the service schedule for your specific vehicle.
                    </span>
                @endslot
                @slot('textMedium')
                ...
                @endslot
            @endcomponent
            <img src="{{ asset('assets/images/servicing_1.png') }}" class="w-full h-auto" alt="" data-aos="zoom-in">
        </div>
    </section>
        ...
        <div class="grid items-center sm:w-max justify-center mx-auto grid-cols-2 md:grid-cols-3 gap-3 sm:gap-x-6 sm:gap-y-10 lg:grid-cols-4 mt-[60px]" data-aos="fade-up">
            ...
            @foreach ($images as $image)
                ...
                @include('components.brand-card', [
                    ...
                    'route' => "/servicing/".  str_replace(".jpeg", "", $image->getFilename()),
                    'index' => $loop->index,
                ])
            @endforeach
        </div>
